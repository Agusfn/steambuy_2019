...
if(isset($_POST["product_type"]) && isset($_POST["product_id"]) && isset($_POST["payment_method"]) && isset($_POST["buyer_name"]) && isset($_POST["buyer_email"])) {
	...
	if(isset($_POST["buyer_steamurl"])) {
		$buyer_steam_profile = $_POST["buyer_steamurl"];
	} else $buyer_steam_profile = "";
...
validateInitialData($buyer_name, $buyer_email, $payment_method, $buyer_steam_profile);
...
if($session_auth) {
	...
	if($product_exists = $purchase->checkProductPurchasable($product_type, $product_id)) {
		...
		if($product_type == 1 && $productData["product_sellingsite"] == 1 && $productData["product_has_limited_units"] == 0 && $buyer_steam_profile == "") {
			header("Location: ../");
			exit;
		}
...
if($session_auth && $product_exists && $productArsPrices != false && !$priceChange && (!$sentCoupon || ($sentCoupon && $validCoupon))) {
	...
	if(isset($_POST["remember_data"])) {
		remember_client_info($buyer_name, $buyer_email, $buyer_steam_profile);
	} else {
		forget_client_info();
	}	
