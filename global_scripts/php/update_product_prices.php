...
define("override_steam_sales_start", strtotime("2017-07-03 05:00:00")); // A partir de este momento se muestra la fecha de fin de oferta de todos los juegos en oferta, en la fecha customizada
define("override_steam_sales_custom_end", strtotime("2017-07-03 22:00:00"));  // Fecha manual de fin de oferta que indican los juegos. A partir de este momento se ignoran todas las ofertas de Steam
define("override_steam_sales_real_end", strtotime("2017-07-05 16:00:00")); // Acá se dejan de ignorar las ofertas de Steam
	...
	if($product->loadError == 0) {
		...
		if($prices["error"] == 0) {
			...
			{	
				...
				if($pData["product_has_limited_units"] == 0) 
				{ 

					$sql2 = "UPDATE `products` SET `product_has_customprice` = 1, `product_customprice_currency` = 'ars', `product_external_limited_offer` = 0, 
					`product_external_offer_endtime` = '0000-00-00 00:00:00', `product_listprice` = 0, `product_finalprice` = ".($prices["product_finalprice"]*1.25).", `product_steam_discount_price`=0, `product_update_error` = 0 
			...
			else if($prices["product_discount"] == 1) 
			{
				...
				{ 
					...
					$sql2 = "UPDATE `products` SET `product_external_limited_offer` = 1, `product_has_customprice` = 1, `product_customprice_currency`='ars', `product_listprice` = 0, `product_steam_discount_price`=".$prices["product_finalprice"].",
					`product_finalprice` = ".($prices["product_finalprice"]*1.25).", `product_update_error` = 0 WHERE `product_id` = ".$pData["product_id"];
